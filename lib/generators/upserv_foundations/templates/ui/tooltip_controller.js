...
export default class extends Controller {
  ...
  // in case the tool tip is also an action item (ex. open modal), we want to hide the
  // message so that it does not show up over something it shouldn't show up over
  // I had an issue where a tool tip was also a modal trigger and when I clicked the
  // trigger, the tooltip remained above the modal. I was not able to replicate it
  // so I'm not sure if this function is even necessary but if it's not, then come back
  // and fix later. It doesn't cause any harm for now
  click() {
    const toolTipClones = document.getElementsByClassName('tooltip-clone')
  ...
  updateTooltipPosition() {
    ...
    //
    this.messageTargetClone = this.messageTarget.cloneNode(true)
    this.messageTargetClone.classList.add('tooltip-clone')
...
export default class extends Controller {
  ...
  copyElementWithDescendants(element, parent = null) {
    ...
    if (!parent) {
      clone.classList.add('tooltip-clone')
    } else {
